defmodule Issues.TableFormatter do
  ...
  def format_for(column_widths), do: map_join(column_widths, "|", fn width -> "~-#{width}s" end) <> "~n"

  def puts_one_line_in_columns(fields, format), do: :io.format(format, fields)

  def separator(widths), do: map_join(widths, "+", fn width -> List.duplicate("-", width) end)
